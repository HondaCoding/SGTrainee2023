...
/*Cada C'dula/Campo terÿ a respectiva linha:
...
DO WHILE .t.
   ...
   IF LastKey() == 27
      nOpcao := Alert("Deseja continuar?",{'SIM','NAO'})
      IF nOpcao == 2
         loop
      ELSE
         exit
   ...
   nOpcao := Alert("Deseja continuar?",{'SIM','NAO'})
   IF nOpcao == 1
      @ 20,15 say "****** CLIENTE E VENDEDOR REGISTRADOS COM SUCESSO! *****"
      exit
   ENDIF
ENDDO
clear
...
DO WHILE .t.
   ...
   @ 07,15 say "----- Informe a DESCRICAO do PRODUTO 1 ----"
   ...
   IF LastKey() == 27
      nOpcao := Alert("Qual opcao deseja?", {'CONTINUAR', 'CANCELAR', 'PAGAR'})
      IF nOpcao == 2 .and. nOpcao == 3
         exit
      ENDIF
   ENDIF
   ...
   nOpcao := Alert("Deseja continuar?",{'SIM','NAO'})
   IF nOpcao == 1
      @ 20,15 say "*********** PRODUTO 1 REGISTRADO COM SUCESSO! ***********"
   ELSE
      loop
   ENDIF
   ...
   *---------------------- GET PRODUTO 2 --------------------------*/
   @ 07,15 say "----- Informe a DESCRI?A† do PRODUTO 2 ----"
   ...
   *---------------------- GET PRODUTO 3 --------------------------*/
   @ 07,15 say "----- Informe a DESCRI?A† do PRODUTO 3 ----"
